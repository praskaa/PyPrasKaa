# Join Shearwall & Corewall to Structure (Top Priority)

Skrip PyRevit khusus untuk shearwall dan corewall yang secara otomatis menggabungkan dinding struktur ini dengan elemen struktur lainnya dan memastikan shearwall/corewall SELALU menjadi cutting element (memotong elemen lain).

## Deskripsi

Skrip ini mengotomatisasi proses penggabungan shearwall dan corewall dengan elemen struktur lainnya (kolom, balok, fondasi, dll.) dan MEMASTIKAN bahwa shearwall/corewall SELALU memotong semua elemen yang berpotongan dengannya. Menggunakan pendekatan bounding box yang diperluas untuk menangkap elemen yang bersinggungan dan menyentuh.

## Fitur

- **Top Priority Cutting**: Shearwall dan corewall SELALU memotong semua elemen struktur yang berpotongan
- Menggabungkan shearwall/corewall dengan berbagai jenis elemen struktur
- Menangani model group yang berisi elemen struktur
- Termasuk toleransi kecil untuk elemen yang menyentuh
- **Auto Switch Join Order**: Secara otomatis memastikan shearwall/corewall menjadi cutting element
...
### Manajemen Urutan Gabungan (Top Priority Logic)
```python
...
```
Memastikan shearwall/corewall SELALU menjadi cutting element dengan memeriksa dan menyesuaikan urutan gabungan secara otomatis.
...
1. Pilih satu atau lebih **shearwall atau corewall** di Revit (atau biarkan kosong untuk prompt manual)
2. Jalankan skrip dari toolbar PyRevit
3. Skrip akan secara otomatis memastikan shearwall/corewall memotong semua elemen struktur yang berpotongan
4. Tinjau ringkasan hasil
...
- **Shearwall dan corewall akan SELALU memotong semua elemen struktur yang berpotongan**
...
- **Top Priority**: Shearwall/corewall tidak akan pernah terpotong oleh elemen lain
