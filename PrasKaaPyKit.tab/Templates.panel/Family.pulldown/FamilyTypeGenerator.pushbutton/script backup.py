...
class FamilyTypeGenerator:
    ...
    def generate_types_from_csv(self, csv_path):
        ...
        # Step 3: Validate CSV with selected name column
        if not validator.validate(name_column):
        ...
        # Step 5: Process types
        self.output.print_md("\n# ⚙️ Processing Family Types")

        results = self._process_types(validator.rows, matches, name_column)
    ...
    def _process_types(self, csv_rows, parameter_matches, name_column):
        ...
        with Transaction(doc, "Generate Family Types from CSV") as t:
            ...
            with forms.ProgressBar(title='Creating family types...', cancellable=True) as pb:
                for i, row in enumerate(csv_rows):
                    ...
                    result = self._create_single_type(row, parameter_matches, name_column)
                    results['processed'] += 1
    ...
    def _create_single_type(self, csv_row, parameter_matches, name_column):
        ...
        try:
            ...
            if not type_name:
                return {
                    'success': False,
                    'type_name': 'Unknown',
                    'message': 'Missing type name in column "{}"'.format(name_column)
                }
            ...
            for csv_header, value in csv_row.items():
                if csv_header == name_column or not value or not value.strip():
                    continue
