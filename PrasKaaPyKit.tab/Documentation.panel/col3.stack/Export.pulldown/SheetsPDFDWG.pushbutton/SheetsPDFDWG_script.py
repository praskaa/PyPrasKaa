...
if sheets:
    ...
    if is_single_sheet:
        # Single sheet - auto export directly (Individual PDF + DWG mode)
        ...
        with forms.ProgressBar(title='Exporting sheet to PDF and DWG...', cancellable=True) as pb:
            # Export to PDF
            expUtils_exportSheetPdf(pdf_dir, sheets[0], pdf_opts, doc, uidoc, naming_template)
            
            # Export to DWG
            expUtils_exportSheetDwg(dwg_dir, sheets[0], dwg_opts, doc, uidoc, naming_template)
        
        if pb.cancelled:
            forms.alert("Export process cancelled.", title="Script cancelled")
        else:
            forms.alert("Export complete.\n\nPDF: {}\nDWG: {}\n\n{}".format(pdf_dir, dwg_dir, sheet_info), 
