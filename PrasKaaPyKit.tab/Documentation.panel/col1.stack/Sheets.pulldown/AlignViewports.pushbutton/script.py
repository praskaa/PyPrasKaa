    ...
    def align_viewports(self, MainSheet, apply_CropScopeBox = False, overlap = False):
        ...
        for other_viewport in self.viewport_viewplan:
            ...
            with ef_Transaction(doc, "Align viewport"):
                ...
                # [DEBUG] - RESTORE CROP
                if apply_CropScopeBox and crop is not None:
...
# ╩ ╩╩ ╩╩╝╚╝    ╚═╝╚═╝╩ MAIN + GUI
# ==================================================
class AlignViewports(AlignViewportsUI):
    def __init__(self):
        """Main class for GUI and initiating Viewports alignment."""
        # Initialize with UI module - handles all setup automatically
        AlignViewportsUI.__init__(self, "Script.xaml", "Align Viewports")

        # Show dialog if sheets were selected during setup
    ...
    def get_selected_sheets(self):
        ...
        selected_sheets = get_selected_sheets(given_uidoc=uidoc ,title=__title__,
                                              label='Select Sheet to Align Viewports', exit_if_none=False)

        # CANCEL IF NOT ENOUGH SHEETS SELECTED
        if len(selected_sheets) < 2:
            return None  # Return None instead of showing alert
