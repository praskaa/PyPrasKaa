...
PLANES = OrderedDict([
    (DB.PlanViewPlane.TopClipPlane, ([255, 128, 0], "Top Clip Plane", "topplane")),
    ...
    (DB.PlanViewPlane.ViewDepthPlane, ([0, 255, 0], "View Depth Plane", "viewdepth")),
])
...
class Context(object):
    ...
    def __init__(self, view_model):
        ...
        # Updated Units API for Revit 2026 compatibility
        try:
            # Try new API first (Revit 2025+)
            units = doc.GetUnits()
            format_options = units.GetFormatOptions(DB.SpecTypeId.Length)
            self.length_unit = format_options.GetUnitTypeId()
        except:
            try:
                # Fallback to older API
                self.length_unit = (doc.GetUnits()
                                     .GetFormatOptions(DB.UnitType.UT_Length)
                                     .DisplayUnits)
            except:
                # Final fallback
                self.length_unit = DB.DisplayUnitType.DUT_MILLIMETERS
        ...
        try:
            ...
            view_model.unit_label = "mm"  # fallback
    ...
    def context_changed(self):
        ...
        try:
            ...
            # Define which planes to visualize in 3D
            planes_to_visualize = [
                DB.PlanViewPlane.CutPlane,
                DB.PlanViewPlane.ViewDepthPlane
            ]
            
            # Define all planes for which to get elevation data
            all_planes_for_data = [
                DB.PlanViewPlane.TopClipPlane,
                DB.PlanViewPlane.CutPlane,
                DB.PlanViewPlane.BottomClipPlane,
            ...
            if isinstance(self.source_view, DB.ViewPlan):
                # Check for section box - updated for Revit 2026
                try:
                    # Try new parameter access
                    section_box_active = self.active_view.get_Parameter(
                        DB.BuiltInParameter.VIEWER_MODEL_CLIP_BOX_ACTIVE
                    )
                    if section_box_active and section_box_active.AsInteger() == 1:
                ...
                # First, gather elevation data for all relevant planes
                for plane in all_planes_for_data:
                    level_id = view_range.GetLevelId(plane)
                    ...
                    # Updated UnitUtils for Revit 2026
                    try:
                        converted_value = DB.UnitUtils.ConvertFromInternalUnits(
                            plane_elevation,
                            self.length_unit
                        )
                        self.height_data[plane] = round(converted_value, 3)
                    except:
                        # Fallback conversion
                        self.height_data[plane] = round(plane_elevation * 304.8, 3)

                    # Store offset data for editing (3 decimal places)
                    try:
                        offset_value = DB.UnitUtils.ConvertFromInternalUnits(
                            view_range.GetOffset(plane), self.length_unit
                        )
                        self.offset_data[plane] = round(offset_value, 3)
                    except:
                        # Fallback conversion
                        self.offset_data[plane] = round(view_range.GetOffset(plane) * 304.8, 3)

                    self.level_data[plane] = plane_level

                    # Store original data for reset functionality
                    if plane not in self.original_offset_data:
                        self.original_offset_data[plane] = self.offset_data[plane]
                    if plane not in self.original_level_data:
                        self.original_level_data[plane] = level_id

                # Second, create geometry only for planes_to_visualize
                for plane in planes_to_visualize: 
                    # Ensure height data exists for this plane before proceeding
                    if plane not in self.height_data or self.height_data[plane] == "N/A":
                        continue

                    # Retrieve pre-calculated elevation for geometry
                    # This requires converting the string height back to internal units if needed,
                    # or more simply, re-calculating plane_elevation for visualization.
                    # For simplicity, let's re-fetch elevation details for visualization loop.
                    plane_level_geom = self.source_view.Document.GetElement(view_range.GetLevelId(plane))
                    if not plane_level_geom: # Should not happen if height_data was populated
                        continue
                    
                    plane_elevation_geom = (
                        plane_level_geom.ProjectElevation
                        + view_range.GetOffset(plane)
                    )
                    
                    # Only visualize Cut Plane and View Depth Plane in 3D
                    if plane in [DB.PlanViewPlane.CutPlane, DB.PlanViewPlane.ViewDepthPlane]:
                        cut_plane_vertices = [
                            DB.XYZ(c.X, c.Y, plane_elevation_geom) for c in corners
                        ]

                        face_color = get_color_from_plane(plane)
                        edge_color = get_edge_color_from_plane(plane)


                        edges.extend(create_edges(cut_plane_vertices, edge_color))
                        triangles.extend(create_triangles(cut_plane_vertices, face_color))

                # Update view model with all gathered height data
                self.view_model.topplane_elevation = str(self.height_data.get(
                    DB.PlanViewPlane.TopClipPlane, "-"))
                self.view_model.cutplane_elevation = str(self.height_data.get(
                    DB.PlanViewPlane.CutPlane, "-"))
                self.view_model.bottomplane_elevation = str(self.height_data.get(
                    DB.PlanViewPlane.BottomClipPlane, "-"))
                self.view_model.viewdepth_elevation = str(self.height_data.get(
                    DB.PlanViewPlane.ViewDepthPlane, "-"))

                # Set initial values for editing fields
                for plane, (_, _, prefix) in PLANES.items():
                    if plane in self.offset_data:
                        setattr(self.view_model, prefix + "_new_value", str(self.offset_data[plane]))
                    else:
                        setattr(self.view_model, prefix + "_new_value", "")

            else: # Handling for non-ViewPlan (e.g., Section View)
                crop_bbox = self.source_view.CropBox
                ...
                # Updated Transform API for Revit 2026
                try:
                    ...
                    # Fallback transform creation
                    view_direction = self.source_view.ViewDirection
                    translation_vector = view_direction.Multiply(-self.source_view.CropBox.Min.Z)
                    view_dir_transform = DB.Transform.CreateTranslation(translation_vector)
    ...
    def is_valid(self):
        if not can_use_view_as_source(self.source_view):
            ...
            return False
                ...
                not self._check_section_box_active()
        ):
    ...
    def _check_section_box_active(self):
        """Helper method to check section box status with fallback"""
        try:
            param = context.active_view.get_Parameter(
                DB.BuiltInParameter.VIEWER_MODEL_CLIP_BOX_ACTIVE
            )
            return param and param.AsInteger() == 1
        except:
            return False
...
class MainWindow(forms.WPFWindow):
    def __init__(self):
        ...
        self.applyButton.Click += self.apply_changes_click
        self.resetButton.Click += self.reset_values_click
    ...
    def close_window_click(self, sender, e):
        self.Close()
...
def get_color_from_plane(plane):
    """Get face color with solid colors (no transparency)"""
    try:
        # Get RGB values from PLANES dictionary
        rgb = PLANES[plane][0]
        r, g, b = rgb[0], rgb[1], rgb[2]

        # Use solid colors (no transparency) for better visibility
        return DB.Color(
            Convert.ToByte(r),
            Convert.ToByte(g),
            Convert.ToByte(b)
        ...
        print("Error in get_color_from_plane (main try): {}".format(e_main))
        try:
            # Get RGB values from PLANES dictionary
            rgb = PLANES[plane][0]
            r, g, b = rgb[0], rgb[1], rgb[2]

            # Fallback with Revit's Color object
            return DB.Color(
                Convert.ToByte(r),
                Convert.ToByte(g),
                Convert.ToByte(b)
            ...
            print("Error in get_color_from_plane (fallback): {}".format(e_fallback))
            # Ultimate fallback - return a default color
            return DB.Color(255, 0, 0)  # Red fallback


def get_edge_color_from_plane(plane):
    """Get solid black edges for maximum definition"""
    try:
        # Solid black edges, also try with 0 transparency
        return DB.ColorWithTransparency(0, 0, 0, 0)  # Fully opaque black
    except Exception as e_main_edge:
        print("Error in get_edge_color_from_plane (main_try): {}".format(e_main_edge))
        try:
            # Alternative black edge
            color = DB.Color(Convert.ToByte(0), Convert.ToByte(0), Convert.ToByte(0))
            return DB.ColorWithTransparency(color, 0) # Fully opaque black
        except Exception as e_fallback_edge:
            print("Error in get_edge_color_from_plane (fallback): {}".format(e_fallback_edge))
