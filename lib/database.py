...
def invis_style(doc=revit.doc):
    ...
    for gs in DB.FilteredElementCollector(doc).OfClass(DB.GraphicsStyle):
        ...
        if gs.GraphicsStyleCategory.Id == DB.ElementId(-2000064):
            return gs
...
def get_document_model_bics(doc=revit.doc):
    ...
    for category in doc.Settings.Categories:
        if HOST_APP.is_newer_than(2022):
            ...
            # print (type(bic), bic)
        if category.CategoryType==DB.CategoryType.Model and bic!= DB.BuiltInCategory.INVALID and category.Id != DB.ElementId.InvalidElementId:
            built_in_categories.append(bic)
    return built_in_categories


FREQUENTLY_SELECTED_CATEGORIES=[
        BIC.OST_StructuralFraming,      # Structural beams, joists, etc.
        BIC.OST_StructuralColumns,      # Structural columns only
        BIC.OST_Walls,                  # Structural walls
        BIC.OST_StructuralFoundation,   # Structural foundations
        BIC.OST_Floors,                 # Structural floors/slabs
        BIC.OST_DetailComponents,       # Detail components
        BIC.OST_Rebar                   # Structural Rebar     
        ]
