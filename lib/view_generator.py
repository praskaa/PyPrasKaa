...
class ViewGenerator:
    ...
    def get_floor_plan_view_type(self):
        """Get the Structural Plan view family type"""
    ...
    def calculate_walls_bounding_box(self, elements, level):
        """
        Calculate bounding box that encompasses walls relevant to the specified level
        Returns BoundingBoxXYZ for crop region
        """
        if not elements:
            return None

        # Filter walls that are relevant to this level
        relevant_walls = self._filter_walls_by_level(elements, level)

        if not relevant_walls:
            # Fallback to all walls if no walls found for this level
            relevant_walls = elements
    ...
    def _filter_walls_by_level(self, walls, target_level):
        """
        Filter walls that are relevant to the target level
        A wall is considered relevant if:
        - Its base level matches the target level, OR
        - Its top level matches the target level, OR
        - The target level elevation is between wall's base and top elevation
        """
        relevant_walls = []
        ...
        for wall in walls:
            try:
                # Get wall's base and top levels
                ...
                if base_level and base_level.Id == target_level.Id:
                    relevant_walls.append(wall)
                    continue

                if top_level and top_level.Id == target_level.Id:
                    relevant_walls.append(wall)
                    continue

                # Check if target level is between base and top elevations
                if base_level and top_level:
                    base_elev = base_level.Elevation
                    top_elev = top_level.Elevation

                    if base_elev <= target_elevation <= top_elev:
                        relevant_walls.append(wall)

            except:
                # If we can't determine levels, include the wall as fallback
                relevant_walls.append(wall)
