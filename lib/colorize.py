...
# Colorize-only categories (not filterable - for Colorize by Value only)
# These categories support element overrides but not parameter filters
COLORIZE_ONLY_CATEGORIES = [
    DB.BuiltInCategory.OST_GenericAnnotation,  # Generic Annotations
]

# Config option name for colorize-only categories
...
def get_categories_config(doc):
    ...
    default_categories_names = frequent_category_labels()
    categories_names_list= get_config(categories_config, CATEGORIES_CONFIG_OPTION_NAME, default_categories_names)
    return category_labels_to_bic(categories_names_list, doc)


def get_colorize_only_categories_config(doc):
    """
    Get categories for Colorize by Value, including colorize-only categories.
    
    This function returns categories that can be colorized but NOT filtered.
    Includes:
    - Categories from user config (default: frequent model categories)
    - Colorize-only categories (OST_GenericAnnotation - annotations that support overrides but not filters)
    
    Args:
        doc: Revit Document
        
    Returns:
        dict: {localized_label: BuiltInCategory}
    """
    # First get the standard categories from config
    categories_config = get_categories_config(doc)
    
    # Add colorize-only categories
    for bic in COLORIZE_ONLY_CATEGORIES:
        try:
            label = DB.LabelUtils.GetLabelFor(bic)
            # Only add if not already present
            if label not in categories_config:
                categories_config[label] = bic
        except:
            pass
...
def config_category_overrides(doc):
    ...
    prev_cat_overrides = load_configs(categories_config, CATEGORIES_CONFIG_OPTION_NAME, frequent_category_labels())
    category_options = [ChosenItem(x, checked=x in prev_cat_overrides) for x in model_categories_dict(doc)]
...
def set_colour_overrides_by_option(overrides_option, colour, doc):
    ...
    solid_fill_pat_id = get_solid_fill_pat(doc).Id
