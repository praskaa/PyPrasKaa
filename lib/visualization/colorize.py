"""
Color generation and override management utilities for visualization.

Migrated from pyChilizer colorize.py module.
Provides color palette generation, graphic overrides, and configuration management.
"""
...
from utilities.revit_database import get_solid_fill_pat, frequent_category_labels, category_labels_to_bic, model_categories_dict
...
def basic_colours():
    """Return basic color palette (14 colors)."""
    basic_colours = [
        "#40DFFF",
        "#803ABA",
        "#E6B637",
...
def revit_colour(hex):
    """Convert hex color to Revit Color object."""
    rgb = hex_to_rgb(hex)
...
def random_colour_hsv(n):
    """Return random colors based on HSV."""
...
def get_colours(n):
    """Generate a list of Revit Color objects for n categories."""
    if n < 14:
...
def get_categories_config(doc):
    """Get category language-specific labels from config and return a dictionary {Label:BIC}."""
...
def set_colour_overrides_by_option(overrides_option, colour, doc):
    """Apply color overrides based on selected options."""
