...
# Safe log path function with fallback
def get_safe_log_path(new_path, fallback_path):
    """
    Get log path with fallback to old location if new path fails
    """
    try:
        # Try to create the directory for new path
        os.makedirs(os.path.dirname(new_path), exist_ok=True)
        return new_path
    except Exception as e:
        # Fallback to old path if new path fails
        try:
            os.makedirs(fallback_path, exist_ok=True)
            return fallback_path
        except:
            # Ultimate fallback - return the path anyway
            return fallback_path

# Get Documents path using .NET
try:
    import System
...
# New Documents-based paths with fallback to old F: drive paths
def_hookLogs = get_safe_log_path(
    os.path.join(documents_path, 'PrasKaaPyKit', 'hooksLogs'),
    r"F:\1_STUDI\_PrasKaa Python Kit\PrasKaaToolsLogs\hooksLogs"
)
def_revitBuildLogs = get_safe_log_path(
    os.path.join(documents_path, 'PrasKaaPyKit', 'versions.log'),
    r"F:\1_STUDI\_PrasKaa Python Kit\PrasKaaToolsLogs\versions.log"
)
...
def_syncLogPath = get_safe_log_path(
    os.path.join(documents_path, 'PrasKaaPyKit', 'syncTimeLogs'),
    r"F:\1_STUDI\_PrasKaa Python Kit\PrasKaaToolsLogs\syncTimeLogs"
)
def_openingLogPath = get_safe_log_path(
    os.path.join(documents_path, 'PrasKaaPyKit', 'openingTimeLogs'),
    r"F:\1_STUDI\_PrasKaa Python Kit\PrasKaaToolsLogs\openingTimeLogs"
)
def_dashboardsPath = get_safe_log_path(
...
def_familyloadLogPath = get_safe_log_path(
    os.path.join(documents_path, 'PrasKaaPyKit', 'FamilyLoad'),
        ...
        with open(config_path, "r") as company_config_file:
            ...
            for line in configSetting:
                ...
                if " = " in line:
                    ...
                    # Update paths - convert old F: drive paths to Documents paths for portability
                    value = keys[1]
                    old_base_path = r'F:\1_STUDI\_PrasKaa Python Kit\PrasKaaToolsLogs'

                    if value.startswith(old_base_path):
